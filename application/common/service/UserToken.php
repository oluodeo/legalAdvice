...
class UserToken extends Token
{
	...
	function __construct($code)
    {
		$this->userM = new User();
        $this->code = $code;
        $this->wxAppID = config('setting.app_id');
        $this->wxAppSecret = config('setting.app_secret');
        $this->wxLoginUrl = sprintf(
            config('setting.login_url'), $this->wxAppID, $this->wxAppSecret, $this->code);
    }
    ...
    // 只能延迟自有token的过期时间超过7200秒（目前还无法确定，在express_in时间到期后还能否进行微信支付
    ...
    private function grantToken($wxResult)
    {
        ...
        $user = $this->userM->getByOpenid($openid);
...
//		print_r($cachedValue);exit();
		//saveToCache生成key，并存入缓存,返回key
    ...
    {
        $key = self::generateToken();//基类里获取加密key的方法
        $value = json_encode($wxResult);
        $expire_in = config('setting.token_expire_in');//缓存失效时间
